...
class vector
{
  ...
  public:
    ...
    {
        return tiny::find(_element, _end, value);
    }
};
...
inline vector<T>::vector(size_type count, const_reference value)
{
    ...
    _finish = _end = tiny::uninitialized_fill_n(_element, count, value);
}
...
inline vector<T>::vector(const vector &other)
{
    ...
    _finish = _end =
        tiny::uninitialized_copy(other.begin(), other.end(), _element);
}
...
vector<T>::vector(Iter start, Iter finish)
{
    ...
    _finish = _end = tiny::uninitialized_copy(start, finish, _element);
}
...
void vector<T>::assign(Iter start, Iter finish)
{
    ...
    {
        Iter mid = tiny::advance(start, size());  // 拷贝前一半,构造后一半
...
void vector<T>::assign(diff_type count, const_reference value)
{
    ...
    } else  // 3.新元素比原来的元素多
    {
        ...
        _finish = tiny::uninitialized_fill_n(_finish, count - mid, value);
    }
}
...
void vector<T>::reserve(size_type new_capacity)
{
    ...
    if (new_capacity > capacity()) {  // reserve不能使容器变小
        ...
        tiny::uninitialized_copy(_element, _finish, new_elem);
        destroy(_element, _finish);
        Alloc::deallocate(_element);
        _element = new_elem;
...
void vector<T>::resize(size_type new_size)
{
    ...
    if (new_size > old_size) {  // 增大
        ...
        _finish =
            tiny::uninitialized_fill_default(_finish, new_size - old_size);
...
void vector<T>::push_back(const_reference value)
{
    if (_finish == _end) {
        if (capacity() == 0) {
            ...
            pointer newfinish =
                tiny::uninitialized_copy(_element, _finish, newelem);
            destroy(_element, _finish);
            Alloc::deallocate(_element);
            _element = newelem;
            _finish = newfinish;
