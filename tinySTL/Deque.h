...
class _DequeIterator {
   ...
   public:
    ...
    void jump(T** map_node) {
        map_node = map_node;
        start = *map_node;
        finish = *map_node + _buffer_size();
    }
    T& operator*() const { return *cur; }
    T* operator->() const { return &(operator*()); }

    unsigned int operator-(const self& x) const {
        return _buffer_size() * (map_node - x.map_node - 1) + (cur - start) +
               (x.finish - x.cur);
    }
    ...
    self& operator+=(unsigned int n) {
        unsigned int newpos = n + (cur - first);
        if (newpos >= 0 && newpos < _buffer_size())
            cur += n;
        else {
            unsigned int node_pos = newpos > 0
                                        ? newpos / _buffer_size()
                                        : (-newpos - 1) / _buffer_size() - 1;
            jump(node + node_pos);
            cur = first + newpos - node_pos * _buffer_size();
        }
    }
...
template <typename T, unsigned int buf_siz>
Deque<T, buf_siz>::Deque(unsigned int n = 0)
...
void Deque<T, buf_siz>::expand_map(unsigned int num_new_node = 1,
    ...
    if (m_capacity > 2 * new_capacity) {
        ...
        unsigned int new_capacity =
            m_capacity + num_new_node + 2;  //首尾各加一个空node
...
void Deque<T, buf_siz>::clear() {
    for (T** cur_buf = start.node + 1; cur_buf < finish.node; ++cur_buf) {
...
template <typename T, unsigned int buf_siz>
Deque<T, buf_siz>::Iterator Deque<T, buf_siz>::insert(const Iterator& pos,
                                                      const T& value) {
...
template <typename T, unsigned int buf_siz>
Deque<T, buf_siz>::Iterator Deque<T, buf_siz>::erase(const Iterator& pos) {
    unsigned int num_before = pos - start;
    if (num_before < size() / 2) {
        /*前面元素少,前面向后补齐*/
        // move_backward
