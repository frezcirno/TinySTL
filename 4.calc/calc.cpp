...
optr_t dispatch(char op, int lastObj)
{
    switch (op) {
        ...
        case '\0':
            ...
            return EQU;
            break;
        default: errormsg("错误：未知的运算符");
    }
}

double eval(const char *s)
{
    tinySTL::stack<int> opnd;
    tinySTL::stack<optr_t> optr;
    int lastObj = EQU;
    optr.push(EQU);
